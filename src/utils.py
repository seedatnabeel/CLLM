...
def split_data(df, target_column, n_samples_per_class, random_state=42):
    train_data = df.groupby(target_column).apply(lambda x: x.sample(n_samples_per_class, random_state=random_state)).reset_index(level=0, drop=True)
...
def sample_and_split(df_feat, df_label, ns=10, seed=100):
    ...
    if ns > min_class_size:
        raise ValueError(f'n is greater than your smallest class size of {min_class_size}.')

    # Sample n indices from each class
    sampled_indices = df_label.groupby(df_label).apply(lambda x: x.sample(ns, random_state=seed).index)
            ...
            try:
                ...
                if metric == AlphaPrecision:
                    ...
                    if X_ref.shape[0]> data_check.shape[0]:
                        trial_results = _eval_plugin(metric, GenericDataLoader(X_ref.astype(float).sample(data_check.shape[0])) , GenericDataLoader(data_check.astype(float)))
                    elif X_ref.shape[0]< data_check.shape[0]:
                        trial_results = _eval_plugin(metric, GenericDataLoader(X_ref.astype(float)) , GenericDataLoader(data_check.astype(float).sample(X_ref.shape[0])))
                    else:
                        trial_results = _eval_plugin(metric, GenericDataLoader(X_ref.astype(float)) , GenericDataLoader(data_check.astype(float)))
...
def process_gpt(dataset, n_synthetic, temp, gpt_model, ns, seed):
    ...
    df = df.dropna()
    df = df[~df.apply(lambda row: any([isinstance(cell, str) and cell in ['integer', 'float', 'numeric', 'categorical', 'number', 'No', 'Yes', 'continuous', 'age in years', 'string'] for cell in row]), axis=1)]
    ...
    if gpt_model=='gpt4_nocol':
        ...
        example_df.columns = ['feat_' + str(i) for i in range(example_df.shape[1]-1)] + ['target']
...
def compute_success(df):
    ...
    for _, row in df_easy.iterrows():
        ...
        vanilla_accuracy = df_vanilla[df_vanilla['Group'] == base_name]['Accuracy'].values[0]
...
def process_llama(dataset, n_synthetic, temp, llama_model, ns, seed, path='./llama-gen/llama-data'):
    import pickle
